...
c.on('ready', async () => {
  ...
  // Upload currency route + AppSidebar
  for (const [local, remote] of [
    ...
    ['E:\\POLARYON SYSTEM\\POLARYON KUNBUN\\polaryon-system\\backend\\src\\server.ts', '/var/www/polaryon/backend/src/server.ts'],
  ...
  // Update nginx: fix SW (catch errors) + add currency to regex
  const POLARYON_API_REGEX = '^/api/(health|auth/(login|google|desktop-login|verify|register)|users|calendar|kanban|documents|certificates|accounting|audit|sidebar-links|connections|transparency|activity|maintenance|transferegov|bidding|backups|radar|currency)($|/)';

  const swContent = 'self.addEventListener("install",()=>self.skipWaiting());self.addEventListener("activate",()=>self.clients.claim());self.addEventListener("fetch",(e)=>e.respondWith(fetch(e.request).catch(()=>new Response(null,{status:502}))))';
  ...
  const config = `server {
    ...
    # No-op SW (with error handling for CORS requests)
    location = /sw-modern.js {
        add_header Content-Type application/javascript;
        add_header Service-Worker-Allowed /;
        return 200 '${swContent}';
    }
...
`;
  ...
  // Test currency proxy
  const quotes = await x("curl -s --max-time 10 'https://polaryon.com.br/api/currency/quotes' --resolve polaryon.com.br:443:127.0.0.1");
  console.log('\nCurrency proxy:', quotes.substring(0, 200));
