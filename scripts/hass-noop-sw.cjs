...
(async () => {
    ...
    // Deploy config WITHOUT JS sub_filter, with no-op SW, and auth redirect
    const config = `server {
    ...
    # HA redirect (307 preserva metodo HTTP, ao contrario de 302)
    location /auth/ {
    ...
    # No-op Service Worker (impede que SW antigo interfira)
    ...
    # HA assets (proxy direto, sem sub_filter)
    ...
    # HA entry point (sub_filter APENAS em HTML, SEM JS)
...
`;
    ...
    const tests = [
        ['/', 'SPA'],
        ['/api/health', 'API'],
        ['/auth/login_flow', 'Auth POST redirect'], ['/auth/authorize', 'Auth redirect'],
        ['/hass-xxx/', 'HA entry'],
        ['/hass-xxx/static/icons/favicon-192x192.png', 'HA icon'],
        ['/sw-modern.js', 'SW no-op'],
    ];
    for (const [path, label] of tests) {
        ...
        console.log(`  ${label.padEnd(15)} ${code}`);
    }
    // Check SW content
    const sw = await exec("curl -s --max-time 5 --resolve 'polaryon.com.br:443:127.0.0.1' https://polaryon.com.br/sw-modern.js 2>&1");
    console.log('\nSW content:', sw.substring(0, 80) + '...');
