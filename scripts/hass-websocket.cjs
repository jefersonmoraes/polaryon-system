...
(async () => {
    ...
    # HA WebSocket (precisa de Upgrade headers)
    ...
    # === HOME ASSISTANT (path prefix - entry point com sub_filter) ===
...
`;
    ...
    // Verify WebSocket + key paths
    console.log('\n=== Path checks ===');
    const paths = [
        ['/', 'Polaryon SPA', null],
        ['/api/health', 'Polaryon API', null],
        ['/api/websocket', 'HA WebSocket', '101|400|426'],
        ['/auth/authorize', 'HA auth page', null],
        ['/hass-xxx/', 'HA prefix entry', null],
        ['/api/onboarding/installation_type', 'HA onboarding API', null],
    ];
    for (const [path, label, okCodes] of paths) {
        ...
        const ok = okCodes ? okCodes.includes(code) : (code === '200' || code === '302');
        console.log(`  ${ok ? '✅' : '❌'} ${label.padEnd(18)} ${code}`);
    }
