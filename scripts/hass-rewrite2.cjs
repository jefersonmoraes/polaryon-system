...
(async () => {
    ...
    // Write the correct config (without Connection upgrade in HA block)
    const config = `server {
...
`;
    ...
    // Test root (SPA)
    console.log('\n=== Root (SPA) ===');
    console.log(await exec("curl -s -D - -o /dev/null --max-time 5 --resolve 'polaryon.com.br:443:127.0.0.1' https://polaryon.com.br/ 2>&1 | head -5"));

    // Test HA
    console.log('\n=== HA ===');
    console.log(await exec("curl -s -D - -o /dev/null --max-time 5 --resolve 'polaryon.com.br:443:127.0.0.1' https://polaryon.com.br/hass-" + SECRET + "/ 2>&1 | head -5"));

    // HA body
    console.log('\n=== HA body (first 300 chars) ===');
