...
(async () => {
    ...
    # Home Assistant Service Worker (registrado em /sw-modern.js pela onboarding)
    ...
    # HA API paths (prefix match, nao exact) - cobre /api/onboarding/installation_type etc
    ...
    # CRITICO: /api/ catch-all DEPOIS dos paths especificos do HA
    location /api/ {
...
`;
    ...
    // Verify fixes
    console.log('\n=== Check SW ===');
    console.log('/sw-modern.js: ' + await exec("curl -s -D - -o /dev/null --max-time 5 --resolve 'polaryon.com.br:443:127.0.0.1' https://polaryon.com.br/sw-modern.js 2>&1 | grep -i 'content-type\\|http/'"));
    
    console.log('\n/api/onboarding/installation_type:');
    console.log(await exec("curl -s -D - -o /dev/null --max-time 5 --resolve 'polaryon.com.br:443:127.0.0.1' https://polaryon.com.br/api/onboarding/installation_type 2>&1 | head -8"));
    
    console.log('\n/api/health (Polaryon):');
    console.log(await exec("curl -s -o /dev/null -w '%{http_code}' --max-time 5 --resolve 'polaryon.com.br:443:127.0.0.1' https://polaryon.com.br/api/health 2>&1"));
