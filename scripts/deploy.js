...
function deploy() {
    ...
    try {
        ...
        const remoteCmd = `cd /var/www/polaryon && git fetch origin main && git reset --hard origin/main && [ -L dist/download ] && unlink dist/download || true && npx prisma db push --schema backend/prisma/schema.prisma && npm run build && pm2 restart polaryon-backend`;
