...
function deploy() {
    ...
    try {
        ...
        const remoteCmd = `cd /var/www/polaryon && git fetch origin main && git reset --hard origin/main && [ -L dist/download ] && unlink dist/download || true && npm install && npx prisma db push && npm run build && pm2 restart polaryon-backend`;
