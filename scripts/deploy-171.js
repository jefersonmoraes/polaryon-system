...
conn.on('ready', async () => {
    console.log('🔗 Conectado ao VPS (191.252.93.79)');
    try {
        // 1. Upload files
        ...
        if (fs.existsSync(localBlockmapPath)) {
            console.log(`  → ${blockmapName}`);
            await uploadFile(sftp, localBlockmapPath, remotePath + blockmapName);
        }
        if (fs.existsSync(localYamlPath)) {
            console.log(`  → latest.yml`);
            await uploadFile(sftp, localYamlPath, remotePath + 'latest.yml');
        }
        sftp.end();
        console.log('✅ Upload concluído');

        // 2. Deploy backend/frontend
        console.log('🚀 Executando deploy no VPS...');
        const cmd = [
