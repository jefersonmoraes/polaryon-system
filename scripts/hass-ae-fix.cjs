...
(async () => {
    ...
    // Diagnose: check authorize JS
    console.log('=== authorize.js check ===');
    // First find the exact filename
    const html = await exec("curl -sL --max-time 5 --resolve 'polaryon.com.br:443:127.0.0.1' 'https://polaryon.com.br/auth/authorize?response_type=code&client_id=test' 2>&1");
    const authJsRef = html.match(/\/frontend_latest\/authorize\.[^"']+/);
    if (authJsRef) {
        console.log('Referenced in HTML:', authJsRef[0]);
        const jsPath = authJsRef[0];
        const ct = await exec("curl -s -D - -o /dev/null --max-time 5 -H 'Accept-Encoding: identity' --resolve 'polaryon.com.br:443:127.0.0.1' https://polaryon.com.br" + jsPath + " 2>&1 | grep -i 'content-type\\|http/' | head -5");
        console.log('With identity:', ct);
        const ct2 = await exec("curl -s -D - -o /dev/null --max-time 5 --resolve 'polaryon.com.br:443:127.0.0.1' https://polaryon.com.br" + jsPath + " 2>&1 | grep -i 'content-type\\|http/' | head -5");
        console.log('With gzip:', ct2);
    } else {
        console.log('❌ No authorize.js reference found in auth HTML');
        // Show first reference from frontend_latest
        const refs = html.match(/\/frontend_latest\/[^"']+/g);
        if (refs) console.log('frontend_latest refs:', refs.slice(0, 5));
    }

    console.log('\n=== HA direct check ===');
    // Check if HA has the file
    const haRef = html.match(/\/frontend_latest\/([^"']+)/);
    if (haRef) {
        const jsFile = haRef[1];
        const haRes = await exec("curl -s -D /tmp/ha_hdr.txt -o /dev/null --max-time 5 http://127.0.0.1:8123/frontend_latest/" + jsFile + " 2>&1");
        const haHdr = await exec("cat /tmp/ha_hdr.txt");
        console.log('HA direct headers:', haHdr.substring(0, 300));
    }

    // Fix: add Accept-Encoding "" to all HA locations
    console.log('\n=== Applying Accept-Encoding fix to ALL HA locations ===');
    const config = `server {
