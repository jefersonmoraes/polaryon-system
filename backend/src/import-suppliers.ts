...
const ODS_FILE_PATH = path.join(__dirname, '..', '..', 'importar', 'fornecedores.ods');
    ...
    try {
        ...
        if (response.status === 429 || response.status === 403) {
            if (retryCount < MAX_RETRIES) {
                const waitTime = 60000; // Wait 60s default for any block
...
async function main() {
    console.log(`🚀 Iniciando importação com User-Agent e Delay Superior: ${ODS_FILE_PATH}`);
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        console.log(`[${i + 1}/${data.length}] Processando: ${cnpj}`);
        ...
        const areasAtuacao = (row['Área de Atuação'] || '')
            .split(',')
            .map((s: string) => s.trim())
            .filter(Boolean);
        ...
        addContact('Email Planilha 1', row.EMAIL, 'Email');
        ...
        try {
            await prisma.company.create({
                data: {
                    ...
                    comments: row.observação || '',
                    areasAtuacao: areasAtuacao as any,
                    customLink: row.link || '',
                    lastCnpjCheck: new Date(),
                    createdAt: new Date()
                }
            });
            console.log(`  ✅ ${apiData.razao_social} importado.`);
            successCount++;
        } catch (err: any) {
        ...
        // Delay 3s (20 req/min) - More conservative
